...
{
    // 加载并设置航空公司logo
    QPixmap logoPixmap(info.logo);
    if (logoPixmap.isNull()) {
        // 如果图片加载失败，使用默认图片或显示文字
        qWarning() << "Failed to load logo image:" << info.logo;
        // 尝试加载默认图片
        logoPixmap = QPixmap(":/images/images/logo1.png");
        if (logoPixmap.isNull()) {
            // 如果默认图片也加载失败，创建一个简单的占位符
            logoPixmap = QPixmap(40, 40);
            logoPixmap.fill(Qt::lightGray);
        }
    }
    ui->labelLogo->setPixmap(logoPixmap.scaled(40, 40, Qt::KeepAspectRatio, Qt::SmoothTransformation));
...
void FlightItemWidget::showFlightDetail()
{
    // 简单的航班详情显示方法
    try {
        FlightDetailWidget* detailWidget = new FlightDetailWidget(m_flightInfo, this);
        detailWidget->setWindowTitle(QString("航班详情 - %1").arg(m_flightInfo.flightNo));
        detailWidget->setAttribute(Qt::WA_DeleteOnClose, true);
        detailWidget->setModal(true);
        detailWidget->exec();
    } catch (...) {
        QMessageBox::warning(this, "错误", "无法显示航班详情");
    }
}
...
void FlightItemWidget::mousePressEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton) {
        // 发射信号，让外部处理详情显示（这样可以传递日期等参数）
        emit clicked(m_flightInfo);
    }
