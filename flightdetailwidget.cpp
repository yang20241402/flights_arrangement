...
void FlightDetailWidget::on_reserveBtn_clicked(){

        int userId = 1; // 替换为实际用户ID
        PassengerDialog *dlg = new PassengerDialog(userId, this);
        dlg->setBuyTicketMode(true); // 开启购票模式
        // 绑定选中信号，获取选择结果
        connect(dlg, &PassengerDialog::passengerSelected, this, [=](const PassengerInfo &info){
            if (info.isValid) {
                qDebug() << "选中乘机人：" << info.name << info.idCard;
                // 这里可以处理选中后的逻辑（如填充到订单表单）
            }
        });
        dlg->exec();
        dlg->deleteLater();
